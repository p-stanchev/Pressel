...
#[test]
fn folded_rans_entropy_roundtrip() {
    let bytes = gradient(17, 9);
    let encoded = encode_payload(6, &bytes).unwrap();
    let decoded = decode_payload(6, &encoded, bytes.len()).unwrap();
    assert_eq!(decoded, bytes);
}

#[test]
fn context_split_folded_entropy_roundtrip() {
    let bytes = gradient(17, 9);
